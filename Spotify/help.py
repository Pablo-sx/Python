...
def show_song(page, x, y, title, artist):
    frame = tk.Frame(page, height=200, width=150, bg=root_color)
    frame.place(x=x, y=y)
    ...
    # Dodanie tytułu utworu
    title_label = tk.Label(frame, text=title, bg=root_color, fg="white", font=("Arial", 12))
    title_label.pack(pady=2)

    # Dodanie nazwy wykonawcy
    artist_label = tk.Label(frame, text=artist, bg=root_color, fg="white", font=("Arial", 10, "italic"))
    artist_label.pack(pady=2)

    # Dodanie przycisku "Lubię to"
    like_btn = tk.Button(frame, image=like, bg=root_color, bd=0, activebackground=root_color)
    like_btn.image = like
    like_btn.pack(pady=2)
...
def home_page():
    query = """
        ...
        WHERE l.user_id = 1 -- Zmień '1' na ID użytkownika
        GROUP BY s.type
        ORDER BY COUNT(*) DESC
        LIMIT 1;
    """
    ...
    if result:
        ...
        query2="""SELECT DISTINCT songs.title, artists.name
        FROM songs
        INNER JOIN albums ON songs.album_id = albums.album_id
        INNER JOIN artists ON albums.artist_id = artists.artist_id
        WHERE songs.type = %s
        LIMIT 5;"""
        mycur.execute(query2, (most_liked_type,))
        result2 = mycur.fetchall()
    else:
        result2 = []

    home_page_frame = tk.Frame(page_frame, bg=root_color)
    tk.Label(home_page_frame, text="Home Page", font=('arial', 24), bg=root_color, fg="white").pack(pady=10)

    if result2:
        tk.Label(home_page_frame, text=f"Utwory typu: {most_liked_type}", font=('arial', 18), bg=root_color, fg="white").pack(pady=5)
        
        songs_frame = tk.Frame(home_page_frame, bg="gray")
        songs_frame.place(x=100, y=100)
        
    x = 100
    y = 175
    lb1 = tk.Label(home_page_frame, text="Polecane utwory", font=("Arial", 20), bg=root_color, fg="white")
    ...
    for row in result2:
        title = row[0]
        artist = row[1]
        show_song(page=home_page_frame, x=x, y=y, title=title, artist=artist)
        x += 200

    else:
        tk.Label(home_page_frame, text="Brak danych do wyświetlenia.", font=('arial', 16), bg=root_color, fg="white").pack(pady=20)
...
def library_page():
    ...
    # Pobierz wszystkie song_id dla polubionych utworów użytkownika
    query = "SELECT song_id FROM likes WHERE user_id=%s"
    mycur.execute(query, (user_id,))
    liked_songs = mycur.fetchall()
    ...
    # Przechodzimy przez wszystkie polubione utwory
    for song in liked_songs:
        song_id = song[0]

        # Pobieramy szczegóły utworu na podstawie song_id
        query2 = """
        ...
        """
        mycur.execute(query2, (song_id,))
        song_details = mycur.fetchone()

        if song_details:
            title = song_details[0]  # Tytuł utworu
            artist = song_details[1]  # Nazwa wykonawcy

            # Wyświetlamy utwór w widoku
            show_song(page=library_page_frame, x=x, y=y, title=title, artist=artist)
            x += 200  # Przesunięcie pozycji dla kolejnego utworu

    library_page_frame.pack(fill=tk.BOTH, expand=True)

    
def browse_page():
    # Pierwsze zapytanie SQL: najczęściej polubiony typ piosenki
    query = """
        SELECT s.type
        FROM likes l
        JOIN songs s ON l.song_id = s.song_id
    ...
    if result:
        ...
        # Drugie zapytanie SQL: pobierz 5 utworów tego typu
        query2 = """
            SELECT DISTINCT songs.title, artists.name
            FROM songs
            INNER JOIN albums ON songs.album_id = albums.album_id
            INNER JOIN artists ON albums.artist_id = artists.artist_id
            WHERE songs.type = %s
    ...
    tk.Label(browse_page_frame, text="Home Page", font=('arial', 24), bg=root_color, fg="white").pack(pady=10)

    if result2:
        tk.Label(browse_page_frame, text=f"Utwory typu: {most_liked_type}", font=('arial', 18), bg=root_color, fg="white").pack(pady=5)
        
        songs_frame = tk.Frame(browse_page_frame, bg="gray")
        songs_frame.place(x=100, y=100)
        
        x = 100
        y = 175
        lb1 = tk.Label(browse_page_frame, text="Polecane utwory", font=("Arial", 20), bg=root_color, fg="white")
        lb1.place(x=100, y=125)

        # Wyświetlanie polecanych utworów
        for row in result2:
            title = row[0]
            artist = row[1]
            show_song(page=browse_page_frame, x=x, y=y, song_title=title, artist_name=artist, song_id=None, user_id=user_id)
            x += 200
    else:
        tk.Label(browse_page_frame, text="Brak danych do wyświetlenia.", font=('arial', 16), bg=root_color, fg="white").pack(pady=20)
