...
def show_song(page, x, y, song_title, artist_name, song_id, user_id):
    ...
    def browse_album():
        ...
        select_button = tk.Button(browse_window, text="Wybierz album", command=select_album, bg=purple_color, bd=0, font=("Calibri", 12,"bold"),)
...
def home_page():
    ...
    welcome_label = tk.Label(
        home_page_frame,
        text=f"Witaj, {user_name[0]}!",  # user_name - zmienna przechowująca nazwę użytkownika
        font=("Arial", 30, "bold"),
        bg=root_color,
        fg="white"
    )
    ...
    # Pobieranie najczęściej lubianego typu muzyki
    ...
    # Pobieranie niepolubionych utworów tego samego typu
...
def add_album_form(parent_frame):
    ...
    tk.Button(add_window, text="Zapisz",                 bg=purple_color,
                bd=0,
...
def view_songs(album_id, album_title):
    ...
    if songs:
        for song_id, song_title, artist_name in songs:
            ...
            def delete_song(song_id, song_frame):
                mycur.execute("DELETE FROM songs WHERE song_id = %s", (song_id,))
                connection.commit()

                song_frame.destroy()

            delete_button = tk.Button(song_frame, text="Usuń", bg="red", fg="white", command=lambda song_id=song_id, song_frame=song_frame: delete_song(song_id, song_frame))
    ...
    tk.Button(songs_window, text="Zamknij", bg="red", fg="white", command=songs_window.destroy).pack(pady=10)
